...
it('cannot start a session while another connection holds the feed, so it sees no key in flight', function (): void {
    ...
    try {
        ...
        expect($first->feedCursor)->toBe(
            FleetEvent::query()->where('type', FleetEventType::SessionEnrolled->value)->max('id')
        );
        ...
        expect(FleetEvent::query()->where('type', FleetEventType::SessionEnrolled->value)->count())->toBe(1)
