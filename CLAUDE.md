...
- **Every migration carries a date prefix, and `tests/MigrationPrefixGuardTest.php` refuses one
  that does not.** Laravel runs migrations in filename order and digits sort before letters, so an
  ...
  with two rows naming one table, which nothing reads.
