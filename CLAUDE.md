...
- **Nothing reads from Slack, and a test enforces it.** The package talks HTTP from exactly two files, and `tests/SlackMirrorTest.php` fails on a third: one POST in `Jobs\MirrorEventToSlack`, and `Support\GitHubApp`, whose every request goes to `https://api.github.com`. A read from Slack would let a coordination decision depend on Slack being up and honest.
- **Core reads nothing from GitHub that decides anything -- #318's rule -- with one bounded exception, #383: display-only counts.** `Support\GitHubApp` fetches each lane board repository's open-issue count through a GitHub App (Issues read-only, plus the mandatory Metadata), every five minutes, for the meters. It is scheduled last, in the background, and without overlapping itself, because it waits on GitHub and a foreground event would hold the coordination checks behind it; and it looks installations up per account (`/users/{owner}/installation`) rather than listing them, since a public App's installation list is anyone's to grow. A count is a display for people and decides nothing; when GitHub cannot be reached the meter reads unreadable, which is what #339 built it to say. **The exception covers those counts and nothing else.** Anything that frees a lane, places work, or changes a task's state keeps learning from the webhook alone, so a second caller of `GitHubApp` for any other purpose is the rule lapsing, not the exception growing. A failed fetch stores no reading -- never a zero -- and `Support\BacklogFetches` records why for `robot-council:doctor`. A fetched reading has a null `reported_by`, which is what marks it fetched; `2026_09_25_000002_allow_sessionless_robot_council_backlog_readings.php` records why that is one field rather than a source column. The App's key, its JWT, and installation tokens never reach a log, an exception message, a page, or the cache in the clear: a token is cached encrypted with the application key, and `tests/BacklogFetchTest.php` asserts each with sentinels. The #318 webhook stays the organization webhook; moving it onto the App is a separate decision.
...
- **CI is one workflow with one required check.** [`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs on every pull request and every push to `main`, with no path filters:
  ...
  - `postgres` runs on ubuntu with PHP 8.5 against a `postgres:17` service container. It runs `vendor/bin/pest --ci` with `DB_CONNECTION=pgsql`, then `vendor/bin/pest --ci --group=cross-connection`.
    ...
    **The budget is 47 minutes and the number is derived** (#278, re-derived for #412). It is **3.1x the worst completed run**, the multiple #278 chose: across 87 completed `postgres` jobs from 2026-09-25 17:36Z to 2026-09-26 09:57Z the worst took **896s** and the median **670s**, which gives 2778s. #278's 22 minutes (from a 426s worst on 2026-09-24) had fallen to 1.5x, and two jobs were killed at it with every test passing, 1346s and 1367s. **The same rule set two other budgets in the same pass:** the `tests` job is **19 minutes**, set by its Windows cells (365s worst over 36 runs, 1.6x under the old 10, where #278 had measured 3.3x), and `mysql` is **17** (319s worst over 39 runs). Every other job is still far inside its 10 or 5 -- `browser` is the closest, at 124s. **Re-derive them together when any overruns**, from at least 20 recent completed runs, and record the figures here and in `ci.yml`.

    **Why the job keeps growing** (#412): read per file from a job's own log, every file costs about **0.37s a test**, because every database test runs `migrate:fresh` -- dropping and re-running about 50 migrations -- on an engine where that is not free. 2,159 tests is about thirteen minutes of it, and no single file is more than 50s. So the budget erodes in step with the test count, and making that per-test setup cheaper is #467.
