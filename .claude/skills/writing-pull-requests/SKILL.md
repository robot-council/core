...
**The bound worth knowing:** `gh` resolves the placeholders from its notion of the *current* repository, which `GH_REPO` and `gh repo set-default` both move. Setting `GH_REPO` to a repository **other than the one this checkout belongs to** redirects them there; setting it to this checkout's own repository is indistinguishable from leaving it unset. So the recipe follows the checkout; it is not a guarantee that no other repository can be reached. Read the response's `repository_url` when it matters.

**That second clause is why this paragraph names no repository.** An example that sets `GH_REPO` to a named repository demonstrates a redirect only to a reader who is somewhere else, and reads as a no-op to a reader standing in it — so in a document carried byte-identically by two repositories, a named example is wrong in one of them by construction rather than by drift. Run it against the *other* tree to see it move.
